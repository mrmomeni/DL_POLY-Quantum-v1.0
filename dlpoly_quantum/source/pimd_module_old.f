...
c            zmass((i-iatm0)*nbeads+k)=rmu*weight((k-1)*natms+i)
            zmass((i-iatm0)*nbeads+k)=1.d0/(rmu*weight((k-1)*natms+i))
...
c**********************************************************************
     ...
     x      +wzz((i-iatm0)*nbeads+k-1)*dble(k-2)/dble(k-1)
      ...
      subroutine unstage_coords(lmsite,idnode,mxnode,natms,imcon,ntpmls,
     x           g_qt4f,g_qt4f_1)
