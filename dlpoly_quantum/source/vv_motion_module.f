...
c     deallocate working arrays
     ...
     x      nchain,nrespa,ntpmls,ntcons,ntshl,keyshl,tstep,taut,sigma,
     x      chit,consv,conint,g_qt4f,g_qt4f_1,engke,tolnce,vircon,
     x      chit_shl,sigma_shl,sigma_nhc)
...
c***********************************************************************
      ...
      real(8) tstep,taut,sigma,chit,consv,conint,engke,tolnce,vircon
      real(8) hstep,qmass_t,heta_nhc,hpeta
      real(8) sigma_nhc,qmass_part
      real(8) g_qt4f,g_qt4f_1
      integer fail(nnn)
      real(8) strkin(9)
...
c  update the position of M-site if water model qtip4p/f requested
        ...
        if(lmsite)then
          ...
          call qtip4pf(idnode,imcon,ntpmls,g_qt4f,g_qt4f_1)
...
c  Redistribute the M-site forces if water model qtip4p/f requested
        ...
        if(lmsite)then
          ...
          call qt4_force_redist(ntpmls,g_qt4f,g_qt4f_1)
...
c     deallocate working arrays
     ...
     x    (safe,lshmov,lmsite,isw,idnode,mxnode,natms,imcon,nscons,
     x    nchain,nrespa,ntpmls,ntcons,ntpatm,ntshl,keyshl,tstep,taut,
     x    taup,sigma,sigma_nhc,sigma_volm,alpha_volm,g_qt4f,g_qt4f_1,
     x    virtot,vircon,virlrc,press,volm,chit,consv,
     x    conint,engke,elrc,tolnce,chit_shl,sigma_shl)
...
c***********************************************************************
      ...
      logical safe,lshmov,newjob,lmsite
      integer isw,idnode,mxnode,natms,imcon,nscons,ntcons,ntpatm
      integer i,j,k,iatm0,iatm1,mxiter,iter,kk
      integer nchain,nrespa,ntpmls
      real(8) tstep,taut,taup,chit,consv,conint,engke,elrc,tolnce
      real(8) hstep,sigma,qmass_p,qmass_t
      real(8) sigma_nhc,qmass_baro,qmass_part
      real(8) volm_mass,sigma_volm,alpha_volm,v_epsilon
      real(8) g_qt4f,g_qt4f_1
      real(8) press,volm,virtot,vircon,virlrc
      real(8) volm0,elrc0,virlrc0,vzero,scale
      ...
      save qmass_p,qmass_t,qmass_baro,qmass_part,volm_mass
...
c     integrate and apply nhc barostat
     ...
     x     hstep,volm_mass,qmass_p,qmass_baro,taup,v_epsilon)

c     integrate and apply nhc thermostat
