...
c     Start 1st Suzuki-Yoshida scheme
     ...
     x     exp((-weight(nw)*dt8/nrespa)*peta(j+1)/qmass_part)
           ...
           if (j.eq.1) then
           peta(j) = peta(j) + (weight(nw)*dt4/nrespa)*(engke-dNkT)
           else
           peta(j) = peta(j) + 
...
c       do j=1,nchain
c          write(nrite,"('nhc',i6,2e12.4)")j,eta_nhc(j),peta(j) 
c       enddo   
       ENDDO
     ...
     x     tstep,volm_mass,qmass_p,qmass_baro,taup,v_epsilon)
...
c*********************************************************************
      ...
      integer nw,nsy,respa,nrespa,nchain
      parameter (nsy=7)
      real(8) engke,sigma,tstep,qmass_p,scale,taup
...
c     Start 1st Suzuki-Yoshida scheme
     ...
     x     *((0.5d0*volm_mass*v_epsilon**2)-kT)
           else
           pksi(j) = pksi(j) + 
...
c     Thermostat the velocities
      ...
      scale=exp((-weight(nw)*dt2/nrespa)*pksi(1)/qmass_p)
