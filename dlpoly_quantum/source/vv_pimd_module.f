...
c***********************************************************************
      ...
      use pimd_module,    only : zmass,etx,ety,etz,pcx,pcy,pcz,
...
c     update thermostat momenta - 1/4 step
        ...
        do i=1,iatm1-iatm0
          ...
          pcx(1,i)=pcx(1,i)+qstep*(pxx(i)**2/zmass(i)-boltz*temp)
          pcy(1,i)=pcy(1,i)+qstep*(pyy(i)**2/zmass(i)-boltz*temp)
          pcz(1,i)=pcz(1,i)+qstep*(pzz(i)**2/zmass(i)-boltz*temp)
...
c     integrate bead positions - 1 step
        ...
        do i=1,iatm1-iatm0
          ...
          uxx(i)=uxx(i)+(tstep/zmass(i))*pxx(i)
          uyy(i)=uyy(i)+(tstep/zmass(i))*pyy(i)
          uzz(i)=uzz(i)+(tstep/zmass(i))*pzz(i)
          
        enddo
        
c     unstage coordinates
        
        call unstage_coords(idnode,mxnode,natms)
...
c     calculate  kinetic tensor and kinetic energy
     ...
     x  (isw,idnode,mxnode,natms,tstep,taut,temp,engke,engthe,chi,uuu)
...
c     update thermostat momenta - 1/2 step
        ...
        do k=1,nbeads
          ...
          do i=k,iatm1-iatm0,nbeads
            ...
            pcx(1,i)=elam*pcx(1,i)+qstep*(pxx(i)**2/zmass(i)-boltz*temp)
            pcy(1,i)=elam*pcy(1,i)+qstep*(pyy(i)**2/zmass(i)-boltz*temp)
            pcz(1,i)=elam*pcz(1,i)+qstep*(pzz(i)**2/zmass(i)-boltz*temp)
...
c     thermostat the bead momentum
          ...
          do i=k,iatm1-iatm0,nbeads
            ...
            pcx(1,i)=elam*(pcx(1,i)+qstep*(pxx(i)**2/zmass(i)-
     x        boltz*temp))
            pcy(1,i)=elam*(pcy(1,i)+qstep*(pyy(i)**2/zmass(i)-
     x        boltz*temp))
            pcz(1,i)=elam*(pcz(1,i)+qstep*(pzz(i)**2/zmass(i)-
     x        boltz*temp))
...
c     integrate bead positions - 1/2 step
        ...
        do i=1,iatm1-iatm0
          ...
          uxx(i)=uxx(i)+(hstep/zmass(i))*pxx(i)
          uyy(i)=uyy(i)+(hstep/zmass(i))*pyy(i)
          uzz(i)=uzz(i)+(hstep/zmass(i))*pzz(i)
...
c     update thermostat momenta - full step
     ...
     x        tstep*(pxx(i)**2/zmass(i)-boltz*temp)+
     x        rstep*qqq*gssrnd(noskip,uuu))
            pcy(1,i)=facp*(pcy(1,i)*facm+
     x        tstep*(pyy(i)**2/zmass(i)-boltz*temp)+
     x        rstep*qqq*gssrnd(noskip,uuu))
            pcz(1,i)=facp*(pcz(1,i)*facm+
     x        tstep*(pzz(i)**2/zmass(i)-boltz*temp)+
     x        rstep*qqq*gssrnd(noskip,uuu))
...
c     calculate  kinetic tensor and kinetic energy
      ...
      subroutine pimd_nvt_nhc(isw,idnode,mxnode,natms,tstep,taut,
     x  temp,engke,engthe)
...
c     integrate thermostat momenta - 1/2 step
     ...
     x        hstep*(pcz(nchain-1,i)**2/qqq(k)-boltz*temp)
          ...
          pcx(1,i)=pcx(1,i)+hstep*(pxx(i)**2/zmass(i)-boltz*temp)
          pcy(1,i)=pcy(1,i)+hstep*(pyy(i)**2/zmass(i)-boltz*temp)
          pcz(1,i)=pcz(1,i)+hstep*(pzz(i)**2/zmass(i)-boltz*temp)
